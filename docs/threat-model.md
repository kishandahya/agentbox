...
| **Container escape** | cap_drop ALL then cap_add minimum for sshd (CHOWN, DAC_OVERRIDE, FOWNER, SETUID, SETGID, SYS_CHROOT, AUDIT_WRITE, KILL). no-new-privileges, no Docker socket, no SYS_ADMIN, no SYS_PTRACE. | Docker escape 0-days exist. CVE-2024-21626 (Leaky Vessels) was a recent example. This is namespace isolation, not a hypervisor boundary. |
...
| **Privilege escalation** | no-new-privileges prevents setuid. All capabilities dropped except the minimum set needed for sshd (no NET_RAW, no SYS_ADMIN, no NET_ADMIN). Agent user has passwordless sudo inside the container (needed for apt install), but this is contained by the restricted capabilities at the container level. | sudo inside the container gives root inside the namespace, but the namespace has very limited capabilities. A kernel vulnerability could still escalate. |
